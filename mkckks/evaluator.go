...
func (eval *Evaluator) MulRelin(op0, op1 *Ciphertext, rlkSet *mkrlwe.RelinearizationKeySet, ctOut *Ciphertext) {
	...
	x := mkrlwe.NewSwitchingKey(eval.params.Parameters)
	y := mkrlwe.NewSwitchingKey(eval.params.Parameters)
	...
	for id := range idset0.Value {
		...
		for i := 0; i < beta; i++ {
			ringQP.MulCoeffsMontgomeryAndAddLvl(level, levelP, d.Value[i], eval.swkPool.Value[i], x.Value[i])
		}
	}
	...
	for id := range idset1.Value {
		...
		for i := 0; i < beta; i++ {
			ringQP.MulCoeffsMontgomeryAndAddLvl(level, levelP, b.Value[i], eval.swkPool.Value[i], y.Value[i])
		}
	}
	...
	for id := range idset.Value {
		if !idset0.Has(id) { //op0_j = 0
			...
			ringQ.MulCoeffsMontgomeryLvl(level, ctOut.Value[id], op1.Value[id], ctOut.Value[id])
		} else if !idset1.Has(id) { //op1_j = 0
			ringQ.MFormLvl(level, op1.Value["0"], ctOut.Value[id])
			ringQ.MulCoeffsMontgomeryLvl(level, ctOut.Value[id], op0.Value[id], ctOut.Value[id])
		} else { //general case
			ringQ.MFormLvl(level, op0.Value["0"], eval.polyQPool[0])
			ringQ.MFormLvl(level, op0.Value[id], eval.polyQPool[1])

			ringQ.MulCoeffsMontgomeryLvl(level, eval.polyQPool[0], op1.Value[id], ctOut.Value[id])
			ringQ.MulCoeffsMontgomeryAndAddLvl(level, eval.polyQPool[1], op1.Value["0"], ctOut.Value[id])
		}
	}

	//ctOut_j <- ctOut_j + op1_j + Inter(op1_j, x)
	for id := range idset1.Value {
		ringQ.MFormLvl(level, op1.Value[id], eval.polyQPool[0])
		ks.InternalProduct(level, eval.polyQPool[0], x, eval.polyQPool[0])
		ringQ.AddLvl(level, ctOut.Value[id], eval.polyQPool[0], ctOut.Value[id])
		ringQ.AddLvl(level, ctOut.Value[id], op1.Value[id], ctOut.Value[id])
	}
